...
import { _print, _print_inline, _print_bold } from "../utils/output";

import ERC20_ABI from "../abis/erc20_abi";
import MINTER_ABI from "../abis/minter_abi";
import SADDLE_SWAP_ABI from "../abis/saddle_swap_abi";
import POOL_MANAGER_ABI from "../abis/pool_manager_abi";
import UNI_ABI from "../abis/uni_abi";
import LIXIR_TOKEN_ABI from "../abis/uni_abi";
import DLP_ABI from "../abis/dlp_abi";
import DLP_DUAL_TOKEN_ABI from "../abis/dlp_dual_abi";
import BALANCER_POOL_ABI from "../abis/balancer_pool_abi";
import ANGLE_POOL_ABI from "../abis/angle_pool_abi";
import BPOOL_TOKEN_ABI from "../abis/bpool_token_abi";
import BPOOL_ABI from "../abis/bpool_abi";
import JAR_ABI from "../abis/jar_abi";
import CTOKEN_ABI from "../abis/ctoken_abi";
import TOKEMAK_VAULT_ABI from "../abis/tokemak_vault_abi";
import GELATO_ABI from "../abis/gelato_abi";
import HARVEST_VAULT_ABI from "../abis/harvest_vault_abi";
import DSTOKEN_ABI from "../abis/dstoken_abi";
import N_TOKEN_ABI from "../abis/n_token_abi";
import SADDLE_LP_TOKEN_ABI from "../abis/saddle_lp_poken_abi";
import CURVE_ABI from "../abis/curve_abi";
import STABLESWAP_ABI from "../abis/stableswap_abi";
...
const lookUpPrices = async function (id_array) {
  const prices = {};
  const resArr = await Bluebird.map(
    chunk(id_array, 50).map((x) => x.join("%2C")),
    async (id_chunk) => {
      return fetch(
        `https://api.coingecko.com/api/v3/simple/price?ids=${id_chunk}&vs_currencies=usd`
      ).then((r) => r.json());
    },
    { concurrency: 5 }
  );
...
const lookUpTokenPrices = async function (id_array) {
  ...
  const resArr = await Bluebird.map(
    ...
    async (id_chunk) => {
      return fetch(
        `https://api.coingecko.com/api/v3/simple/token_price/ethereum?contract_addresses=${id_chunk}&vs_currencies=usd`
      ).then((r) => r.json());
    },
    { concurrency: 5 }
  );
...
const getBlockNumberFromTimestamp = async function (timestamp) {
  return fetch(
    `https://api.etherscan.io/api?module=block&action=getblocknobytime&timestamp=${timestamp}&closest=before&apikey=XRFWK1IDBR545CXNJ6NQSYAVINUQB7IDV1`
  )
    .then((r) => r.json())
    .then((r) => r.result);
};
...
const getSourceCode = async function (address) {
  return fetch(
    `https://api.etherscan.io/api?module=contract&action=getsourcecode&address=${address}&apikey=XRFWK1IDBR545CXNJ6NQSYAVINUQB7IDV1`
  ).then((r) => r.json());
};
...
async function getDlpPool(App, dlpPool, tokenAddress, originTokenAddress) {
...
async function getBalancerPool(
  ...
  const tokenCalls = tokens
    .map((t) => [pool.getNormalizedWeight(t), pool.getBalance(t)])
    .flat();
  const calls = [
    pool.decimals(),
    pool.symbol(),
...
async function getAngleToken(app, angelToken, address, stakingAddress) {
  const calls = [
    angelToken.decimals(),
    angelToken.totalSupply(),
    angelToken.name(),
    angelToken.symbol(),
    angelToken.balanceOf(stakingAddress),
    angelToken.balanceOf(app.YOUR_ADDRESS),
    angelToken.poolManager(),
  ];
  const [decimals, totalSupply, name, symbol, staked, unstaked, poolManager] =
    await app.ethcallProvider.all(calls);
  const poolManagerContract = new ethcall.Contract(
    poolManager,
    POOL_MANAGER_ABI
  );
...
async function getNToken(app, token, address, stakingAddress) {
  ...
  const [staked, unstaked, name, symbol, totalSupply] =
    await app.ethcallProvider.all(calls);
  return {
    address,
    name,
...
async function getStoredToken(app, tokenAddress, stakingAddress, type) {
  switch (type) {
    ...
    case "doualDlp":
      const doualDlpPool = new ethcall.Contract(
        tokenAddress,
        DLP_DUAL_TOKEN_ABI
      );
      return await getDodoDualPoolToken(
        app,
        doualDlpPool,
        tokenAddress,
        stakingAddress
...
async function getToken(app, tokenAddress, stakingAddress) {
  ...
  try {
    ...
    const lixir = await getLixirPool(
      app,
      lixirPool,
      tokenAddress,
      stakingAddress
  ...
  try {
    ...
    // const _tokenByIndex = await app.ethcallProvider.all([
    //   nPool.tokenByIndex(0),
    // ]);
  ...
  try {
    ...
    const doualDlpPool = await getDodoDualPoolToken(
      app,
      pool,
      tokenAddress,
      stakingAddress
  ...
  try {
    ...
    const anglePool = await getAngleToken(
      app,
      angle,
      tokenAddress,
      stakingAddress
  ...
  try {
    ...
    const dlpPool = await getDlpPool(
      app,
      pool,
      tokenAddress,
      originTokenAddress,
  ...
  try {
    ...
    const res = await getTokemakVault(
      app,
      vaultTokemak,
      tokenAddress,
      stakingAddress
...
function getUniPrices(tokens, prices, pool, chain = "eth") {
  ...
  else if (pool.name.includes("MISTswap LP Token"))
    stakeTokenTicker += " MistSwap LP";
  else if (pool.name.includes("TANGOswap LP Token"))
    stakeTokenTicker += " TangoSwap LP";
  ...
  else if (pool.symbol.includes("LOVE LP"))
    stakeTokenTicker += " Love Boat Love LP Token";
  else if (pool.symbol.includes("Proto-LP"))
    stakeTokenTicker += " ProtoFi LP Token";
  else if (pool.symbol.includes("SOUL-LP"))
    stakeTokenTicker += " Soulswap LP Token";
  else if (pool.symbol.includes("lv_")) stakeTokenTicker += " Lixir LP Token";
  else if (pool.symbol.includes("LOOT-LP"))
    stakeTokenTicker += " Loot LP Token";
  else if (pool.symbol.includes("MIMO-LP"))
    stakeTokenTicker += " Mimo LP Token";
  else if (pool.symbol.includes("HLP"))
    stakeTokenTicker += " Hades Swap LP Token";
  else if (pool.symbol.includes("MOCHI-LP"))
    stakeTokenTicker += " Mochi LP Token";
  else if (pool.symbol.includes("SMUG-LP"))
    stakeTokenTicker += " Smug LP Token";
  else if (pool.symbol.includes("VVS-LP")) stakeTokenTicker += " VVS LP Token";
  else if (pool.symbol.includes("Crona-LP"))
    stakeTokenTicker += " Crona LP Token";
  else if (pool.symbol.includes("Genesis-LP"))
    stakeTokenTicker += " Genesis LP Token";
  else if (pool.symbol.includes("Wagyu-LP"))
    stakeTokenTicker += " Wagyu LP Token";
  else if (pool.symbol.includes("OLP")) stakeTokenTicker += " Oolong LP Token";
  else if (pool.symbol.includes("TLP"))
    stakeTokenTicker += " Trisolaris LP Token";
  else if (pool.symbol.includes("SCLP"))
    stakeTokenTicker += " SwapperChan LP Token";
    ...
    print_price(chain = "eth", decimals, customURLs) {
      ...
      if (customURLs) {
        ...
        const poolUrl = pool.is1inch
          ...
          ? `https://analytics.swapperchan.com/pairs/${pool.address}`
          : pool.name.includes("Ubeswap")
          ? `https://info.ubeswap.org/pair/${pool.address}`
          ...
          ? `https://cchain.explorer.avax.network/address/${pool.address}`
          ...
          ? `https://v2.info.mimo.exchange/pair/${pool.address}`
          ...
          ? `https://analytics.hadesswap.finance/pairs/${pool.address}`
          : pool.symbol.includes("LOOT-LP")
          ? `https://analytics.lootswap.finance/pair/${pool.address}`
          : pool.symbol.includes("VVS-LP")
          ? `https://vvs.finance/info/farm/${pool.address}`
          : pool.symbol.includes("TLP")
          ? `https://explorer.mainnet.aurora.dev/address/${pool.address}`
          : pool.symbol.includes("Crona-LP")
          ? `https://app.cronaswap.org/info/${pool.address}` //wait for real version
          ...
          ? `https://analytics.mistswap.fi/pair/${pool.address}`
          : pool.symbol.includes("Proto-LP")
          ? `https://polygonscan.com/${pool.address}`
    ...
    pair_links(chain = "eth", decimals, customURLs) {
      ...
      if (customURLs) {
        ...
        const poolUrl = pool.is1inch
          ...
          : pool.name.includes("MISTswap LP Token")
          ? `http://analytics.mistswap.fi/pairs/${pool.address}`
          : pool.symbol.includes("Galaxy-LP")
          ? {
              bsc: `https://bsc-exchange.galaxyfinance.one/#/swap`,
              heco: `https://heco-exchange.galaxyfinance.one/#/swap`,
...
function getBalancerPrices(tokens, prices, pool) {
  ...
  var poolPrices = pool.poolTokens.map(
    (t) => getParameterCaseInsensitive(prices, t.address)?.usd
  );
  ...
  if (missing.length === poolPrices.length) {
    throw new Error("Every price is missing");
  }
  ...
  var tickers = pool.poolTokens.map(
    (pt, i) => `[${poolTokens[i].symbol} ${(pt.weight * 100).toFixed(2)}%]`
  );
  ...
  return {
    ...
    print_contained_price(userStaked) {
      ...
      var userQs = quantities.map(
        (q, i) => `${(q * userPct).toFixed(4)} ${poolTokens[i].symbol}`
      );
    ...
    ...{
      print_price() {
        ...
        if (pool.poolType && pool.poolType === "stable") {
          ...
          poolUrl = `https://www.bunicorn.exchange/#/liquidity/stablecoins/detail/${pairStr.toLowerCase()}/${
            pool.address
          }`;
        ...
        _print(
          `<a href='${poolUrl}' target='_blank'>${
            result.stakeTokenTicker
          }</a> BPT Price: $${formatMoney(lpPrice)} TVL: $${formatMoney(
            result.tvl
          )}`
        );
...
function getWrapPrices(tokens, prices, pool) {
  ...
  if (wrappedToken.token0 != null) {
    ...
    return {
      ...
      print_price() {
        _print(
          `<a href='${poolUrl}' target='_blank'>${pool.symbol}</a> (Wrapped ${
            pool.token.name
          }) Price: $${formatMoney(price)} TVL: $${formatMoney(tvl)}`
        );
        _print(
          `Staked: ${pool.staked.toFixed(4)} ${pool.symbol} ($${formatMoney(
            staked_tvl
...
function getCurvePrices(prices, pool, chain) {
  var price =
    getParameterCaseInsensitive(prices, pool.token.address).usd *
    pool.virtualPrice;
...
function getTriCryptoPrices(prices, pool, chain) {
  ...
  for (let i = 0; i < pool.coins.length; i++) {
    const price = getParameterCaseInsensitive(
      prices,
      pool.coins[i].address
    ).usd;
    if (getParameterCaseInsensitive(prices, pool.address)?.usd ?? 0 == 0) {
      prices[pool.address] = { usd: price };
    }
  ...
  return {
    ...
    print_price() {
      _print(
        `${name} Price: $${formatMoney(price)} Market Cap: $${formatMoney(tvl)}`
      );
      _print(
        `Staked: ${pool.staked.toFixed(4)} ${pool.symbol} ($${formatMoney(
          staked_tvl
...
export {
  lookUpPrices,
  lookUpTokenPrices,
  lookUpPricesHistorical,
  lookUpPricesSevenDays,
  getBlockNumberFromTimestamp,
  getSourceCode,
  getPricesSevenDaysStripped,
  getHistoricalPricesStripped,
  getPrices24HoursStripped,
  getStoredToken,
  getToken,
  getPoolPrices,
  getPoolInfo,
