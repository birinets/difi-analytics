...
import BSC_DLP_ABI from "../abis/bsc_dlp_abi";
import BSC_VAULT_ABI from "../abis/bsc_vault_abi";
import BSC_3POOL_ABI from "../abis/bsc_3pool_abi";
import BSC_SWAP_ABI from "../abis/bsc_swap_abi";
import BUNICORN_STABLE_POOL_ABI from "../abis/bunicorn_stable_pool_abi";
...
import VALUE_LP_ABI from "../abis/vault_lp_abi";
...
async function getBscCurveToken(
  App,
  curve,
  address,
  stakingAddress,
...
async function getBscBalancerPool(
  App,
  pool,
  poolAddress,
  stakingAddress,
...
async function getBscBunicornTokenPool(
  App,
  pool,
  poolAddress,
  stakingAddress,
  ...
  const poolData = await getBscBalancerPool(
    App,
    pool,
    poolAddress,
    stakingAddress,
...
async function getBscBunicornStablePool(
  App,
  pool,
  poolAddress,
  stakingAddress,
  ...
  const token0Balance = window.ethers.BigNumber.from(
    tokenBalances[0]
  ).toString();
  const token1Balance = window.ethers.BigNumber.from(
    tokenBalances[1]
  ).toString();
  const totalTokenBalance =
    parseFloat(token0Balance) + parseFloat(token1Balance);
  for (let i = 0; i < tokens.length; i++) {
    const tokenWeightPercent =
      parseFloat(window.ethers.BigNumber.from(tokenBalances[i]).toString()) /
      totalTokenBalance;

    poolTokens.push({
      address: tokens[i],
      weight: Number(tokenWeightPercent.toString()).toFixed(4),
...
async function getBscStoredToken(App, tokenAddress, stakingAddress, type) {
  switch (type) {
    ...
    case "balancer":
      ...
      return await getBscBalancerPool(
        App,
        bal,
        tokenAddress,
        stakingAddress,
    ...
    case "bunicorn:stable":
      const poolStableContract = new ethers.Contract(
        tokenAddress,
      ...
      return await getBscBunicornStablePool(
        App,
        poolStableContract,
        tokenAddress,
        stakingAddress,
...
async function getBscToken(App, tokenAddress, stakingAddress) {
  ...
  try {
    ...
    const tokens = await bal.getFinalTokens();
    const balPool = await getBscBalancerPool(
      App,
      bal,
      tokenAddress,
      stakingAddress,
  ...
  try {
    ...
    const dlpPool = await getBscDlpPool(
      App,
      pool,
      tokenAddress,
      originTokenAddress,
  ...
  try {
    ...
    const stablePool = await getBscBunicornStablePool(
      App,
      stablePoolContract,
      tokenAddress,
      stakingAddress,
...
//     tokens[rewardTokenAddress] = await getBscToken(
//       App,
//       rewardTokenAddress,
//       stakingAddress
//     );
...
// async function loadBscSynthetixPool(
//   App,
//   tokens,
//   prices,
//   abi,
...
//   if (data.SeedBanks) {
//     let p = await loadMultipleBscSynthetixPools(
//       App,
//       tokens,
//       prices,
//       data.SeedBanks
...
async function getBscPoolInfo(
  ...
  const poolToken = await getBscToken(
    App,
    poolInfo.lpToken ?? poolInfo.token,
    chefAddress
  );
...
const bscTokens = [
  ...
  },
  {
    id: "galaxy-triton",
    symbol: "TRITON",
    contract: "0x9cf4009e62429Db3F57Aa9e7e8E898427cF6865f",
  },
  {
    id: "biswap",
    symbol: "BSW",
  ...
  },
  {
    id: "krown",
    symbol: "KRW",
    contract: "0x1446f3cedf4d86a9399e49f7937766e6de2a3aab",
  },
  {
    id: "pancake-hunny",
    symbol: "HUNNY",
    contract: "0x565b72163f17849832A692A3c5928cc502f46D69",
  },
  {
    id: "dodo",
    symbol: "DODO",
  ...
  },
  {
    id: "galaxy-oberon",
    symbol: "OBERON",
    contract: "0xc979E70611D997Aa109528c6A9aa73D82Eaa2881",
  },
  {
    id: "gem",
    symbol: "GEM",
    contract: "0x9fb4DEF63f8caEC83Cb3EBcC22Ba0795258C988a",
  },
  {
    id: "shell",
    symbol: "SHELL",
    contract: "0x01c16da6E041Cf203959624Ade1F39652973D0EB",
  },
  {
    id: "token-dforce-usd",
    symbol: "USX",
    contract: "0xb5102cee1528ce2c760893034a4603663495fd72",
  },
  {
    id: "synapse-2",
    symbol: "SYN",
    contract: "0xa4080f1778e69467E905B8d6F72f6e441f9e9484",
  },
];
